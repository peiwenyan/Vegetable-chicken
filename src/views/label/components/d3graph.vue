...
<!--        >隐藏</span>-->
<!--      </div>-->
<!--      <a-input @input="searchKeyWords" v-model="keywords" clearable placeholder="请输入内容" />-->
...
<!--        ></span>-->
<!--        <font style='color: #034c6a'>{{ name }}</font>-->
<!--      </div>-->
<!--    </div>-->
    <!-- 绘制右边显示结果 -->
    <div id="info" v-show="selectNodeData.name !== undefined">
      ...
      <a-card class="node-card">
        <div class="clearfix">
          <span style='line-height: 32px;font-weight: bold;color: #034c6a'>{{ selectNodeData.name }}</span>
          <a-button
            @click="btnEdit"
            style="float: right; padding: 3px 0;color: #50596d;font-size: 15px;"
            type="link"
          >编辑</a-button>
        </div>
<!--        <a-divider></a-divider>-->
<!--        <div-->
<!--          v-for="(item, key) in selectNodeData.properties"-->
<!--          :key="item"-->
<!--        >-->
...
<!--        </div>-->
    ...
    <!-- 编辑框 -->
    <a-modal v-model="dialogFormVisible" title="编辑节点" @ok="doEdit">
      <a-form
        :model="temp"
  ...
  methods: {
    ...
    hideNodeOfType (event) {
      if (this.nodes.length === this.data.nodes.length
        ...
        console.log('this.data', this.data.nodes.length, this.data.links.length)
    ...
    d3render () {
      ...
      var node = svg.append("g")
        ...
        .on('mouseleave', event => {
          console.log('这个吗', this.isNodeClicked)
