<template>
  ...
  >
    ...
    <a-form-model-item :label-width="0" style="width: 450px" :wrapper-col="{ span: 24 }">
      <a-button type="primary" icon="search" @click="search">搜索</a-button>
      <a-button type="primary" class="ml-5" @click="weekSearch">一周内</a-button>
      <a-button type="primary" class="ml-5" @click="monthSearch">一个月内</a-button>
      <a-button type="primary" class="ml-5" @click="yearSearch">一年内</a-button>
...
export default {
  ...
  methods: {
    ...
    //   if (today.getDate(0) && today.getDate(-7) && today.length > 0) {
    //     data.beginDate = data.date[0].format('YYYY-MM-DD')
    //     data.endDate = data.date[1].format('YYYY-MM-DD')
    //     delete data.date
    //   } else {
