...
export default class MediaEmbedUI extends Plugin {
  ...
  _setUpDropdown(dropdown, form, command) {
    ...
    dropdown.on('upload', () => {
      console.log('点击upload', editor.config)
      if (editor.config._config.videoUpload) {
        const input = document.createElement('input')
        input.setAttribute('type', 'file')
        input.setAttribute('accept', 'video/mp4')
        input.addEventListener('change', () => {
          editor.config._config.videoUpload(input.files[0]).then(({ url: u }) => {
            u ? (editor.execute('mediaEmbed', u), closeUI()) : alert('上传失败')
          })
        })
        input.click()
      } else {
        alert('请配置自定义上传视频的逻辑，ckeditor不清楚你要将本地文件上传到什么地方')
      }
    })
  ...
  _setUpForm(dropdown, form, command) {
    form.delegate('submit', 'cancel', 'upload').to(dropdown)
